sap.ui.define([
	...
	return Controller.extend("PP.mrpReport.controller.Home", {
		...
		onSuggestLive: function (event) {
			...
			var mrpArea = this.getView().byId("idFieldMRPArea").getValue().toUpperCase();
			var h = [];
			h.push(new sap.ui.model.Filter("Material", sap.ui.model.FilterOperator.EQ, oMultiInput1.getValue().toUpperCase()));
			var xx = new sap.ui.model.Filter({
				filters: h,
				or: true,
				_bMultiFilter: false
		...
		// 		items: []
		// 	};
		// 	// var materialNum = sap.ui.getCore().byId("searchField").getValue();
		// 	var botModelSuggest = new sap.ui.model.json.JSONModel();
		// 	var oView = this.getView();
		// 	var oMultiInput1 = oView.byId("multiInput2");
		// 	for (var mName = 0; mName < objData.items.length; mName++) {
		// 		if (objData.items[mName].Material === materialNum) {
		// 			Data.items.push({
		// 				Material: objData.items[mName].Material,
		// 				MaterialName: objData.items[mName].MaterialName
		// 			});
		// 		}
		// 	}
		// 	botModelSuggest.setData(Data);
		...
		_validateMaterial: function () {
			...
			// }
			// var searchUri =
			// 	"/A_MRPMaterial?$orderby=Material&$select=Material,MaterialName&$filter=( MRPPlant eq '" + plant + "' and MRPArea eq '" + mrpArea +
			// 	"')";
			debugger;
			oDateModel.read("/A_MRPMaterial", {
				async: false,
		...
		onSearch: function (oEvent) {
			...
			if (plant !== "" || mrpArea !== "" || oTokens.length > 0) {
				...
				materialModel.read(oSupplierSet, {
					...
					success: function (data, response) {
						...
						for (var i = 0; i < size; i++) {
							...
							// calculate first positive stock available date
							if (valueSupplierSet[i].MRPElementAvailyOrRqmtDate.toISOString() > new Date().toISOString()) {
							...
							// oMaterialStockIt.MRPAvailableQuantity = valueSupplierSet[i].MRPAvailableQuantity;
				...
				// aMaterialSftyStock.Material = valueSupplierSet[i].Material;
				// oMaterialSftyStock.SafetyStk = parseFloat(oMaterialSftyStock.SafetyStk) + SafetyStk;
				debugger;
				//<- aMaterialSftyStock
				//<- aMaterialStockIt
				...
				if (oDateRange.getValue() !== "") {
					for (var i = 0; i < size; i++) {
						if (aMaterialStockIt[i].MRPElementAvailyOrRqmtDate.toISOString() >= oDateRange.getFrom().toISOString() &&
							...
							// OdataBatchHdr.MRPAvailableQuantity = aMaterialStockIt[i].MRPAvailableQuantity;
				...
				for (var i = 0; i < value.length; i++) {
					count++;
					// value[i].MRPAvailableQuantityDate = value[i].MRPAvailableQuantityDate.toISOString();
