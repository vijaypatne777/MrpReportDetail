	...
	return Controller.extend("PP.mrpReport.controller.Home", {
		...
		onHelpTable: function (e) {
			...
			if (plant !== "" || mrpArea !== "") {
				...
				tDialog.getColumns()[0].setProperty("visible", true);
				var dateModel = new sap.ui.model.json.JSONModel();
				dateModel.setData(Data);
				var serviceUrl = "/OPENSAP/sap/opu/odata/sap/API_PRODUCT_SRV";
				var oDateModel = new sap.ui.model.odata.ODataModel(serviceUrl, true);
				// var entitySet = "A_MRPMaterial";
				var entitySet = "A_ProductPlant";
				debugger;
				...
				if (mrpControl === "") {
					var searchUri =
						...
						// mrpArea +
						// "')";	
						"/A_ProductPlant?$orderby=Product&$select=Product,Plant,MRPType&$filter=( Plant eq '" + plant
				...
				var mrpType = this.getView().byId("idMRPType").getValue().toUpperCase();
				if (mrpType !== "") {
					var searchUri =
						// "/A_MRPMaterial?$orderby=Material&$select=Material,MaterialName,MRPController&$filter=( MRPPlant eq '" + plant +
						// "' and MRPArea eq '" + mrpArea + "' and MRPController eq '" + mrpControl +
						// "')";
						"/A_ProductPlant?$orderby=Product&$select=Product,Plant,MRPType&$filter=( Plant eq '" + plant + "' and MRPType eq '" + mrpType +
		...
		onConfirm: function (oEvent) {
			...
			var itemBkup = oMultiInput1.getTokens();
			size = itemBkup.length;
			oMultiInput1.removeAllTokens();
			for (var i = 0; i < size; i++) {
				if (i > 0) {
					var v = i - 1;
					if (itemBkup[v].mProperties.key === itemBkup[i].mProperties.key) {
						break;
					}
				}

				oMultiInput1.addToken(new sap.m.Token({
					key: itemBkup[i].mProperties.key,
					text: itemBkup[i].mProperties.key
				}));
		...
		onSuggestLive: function (event) {
			...
			var mrpArea = this.getView().byId("idFieldMRPArea").getValue().toUpperCase();
			var h = [];
			h.push(new sap.ui.model.Filter("Product", sap.ui.model.FilterOperator.EQ, event.mParameters.query.toUpperCase()));
			var xx = new sap.ui.model.Filter({
				filters: h,
				or: true,
				_bMultiFilter: false
			});
			var m = new sap.ui.model.Filter("Plant", sap.ui.model.FilterOperator.EQ, plant);
			var d = new sap.ui.model.Filter({
				filters: [m],
				and: true
			});
			...
			oDateModel.read("/A_ProductPlant", {
				async: false,
				filters: [R],
				success: function (data, response) {
					var value = data.results;
			...
			var oItems = oMultiInput1._getTokensList().getItems();
			var count = oMultiInput1._getTokensList().getItems().length;
			debugger;

			for (var dName = 0; dName < Data.items.length; dName++) {
				for (var aName = 0; aName < count; aName++) {
					if (Data.items[dName].Material !== oMultiInput1._getTokensList().getItems()[aName].mProperties.title) {
						oMultiInput1.addToken(new sap.m.Token({
							key: Data.items[dName].Material,
							text: Data.items[dName].Material
						}));
			...
			if (plant !== "" || mrpArea !== "" || oTokens.length > 0) {
				...
				materialModel.read(oSupplierSet, {
					...
					success: function (data, response) {
						...
						// 		oMaterialSftyStock.MRPElementCategoryShortName = valueSupplierSet[i].MRPElementCategoryShortName;
						// 		oMaterialSftyStock.MRPElementOpenQuantity = valueSupplierSet[i].MRPElementOpenQuantity;
